...
class TestMCPServer:
    ...
    async def test_handle_resources_list(self):
        ...
        server.register_resource("file://test1.txt", resource1, "Test 1", "First resource")
        server.register_resource("file://test2.txt", resource2, "Test 2", "Second resource", "application/json")
...
class TestMCPServerIntegration:
    ...
    async def test_server_with_protocol_handler(self):
        ...
        server.register_tool("process", test_tool, {
            "type": "object",
        ...
        response, session_id = await server.protocol_handler.handle_message(init_message)
        ...
        tools_message = JSONRPCMessage(
            jsonrpc="2.0",
            id="tools-1",
            method="tools/list"
        )
        
        response, _ = await server.protocol_handler.handle_message(tools_message, session_id)
        ...
        response, _ = await server.protocol_handler.handle_message(call_message, session_id)
