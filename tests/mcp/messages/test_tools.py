...
def test_tool_creation():
    ...
    tool = Tool(name="search", description="Search for information", inputSchema=schema)
...
def test_structured_content_creation_legacy():
    """Test creating structured content (2025-06-18 feature) - legacy version."""
    data = {
        "temperature": 72.5,
        "humidity": 45,
        "pressure": 1013.25,
...
def test_structured_content_minimal_legacy():
    """Test structured content with minimal fields."""
    content = StructuredContent(type="structured", data={"message": "hello"})
...
def test_tool_result_text_only():
    ...
    result = ToolResult(content=[create_text_content("Hello, world!")], isError=False)
...
def test_tool_result_structured_only():
    ...
    structured = StructuredContent(
        type="structured", data={"result": "success", "value": 42}
    )
...
def test_tool_result_mixed_content():
    ...
    structured_content = [
        StructuredContent(type="structured", data={"score": 0.95, "confidence": "high"})
    ]

    result = ToolResult(
        content=text_content, structuredContent=structured_content, isError=False
    )
...
def test_call_tool_result():
    ...
    result = CallToolResult(content=[create_text_content("Success")], isError=False)
...
def test_create_structured_tool_result_legacy2():
    """Test creating structured tool results - legacy2."""
...
def test_create_structured_tool_result_minimal_legacy2():
    """Test creating structured tool result with minimal parameters - legacy2."""
    data = {"status": "ok"}
...
def test_create_mixed_tool_result():
    ...
    structured_content = [
        StructuredContent(
            type="structured", data={"duration": 1.5, "items_processed": 100}
        )
    ]
...
def test_create_error_tool_result():
    ...
    result = create_error_tool_result("Validation failed", error_data=error_data)
...
def test_validate_tool_result():
    ...
    result2 = ToolResult(
        structuredContent=[StructuredContent(type="structured", data={"test": True})]
    )
...
async def test_tool_registry_call_tool_result_object():
    ...
    async def analyze_handler(args):
        return create_mixed_tool_result(
            text_content=[create_text_content("Analysis complete")],
            structured_content=[
                StructuredContent(type="structured", data={"score": 0.85})
            ],
        )
...
def test_structured_content_serialization_legacy():
    """Test structured content serialization - legacy."""
    content = StructuredContent(
        type="structured",
        data={"key": "value"},
        schema={"type": "object"},
...
def test_tool_result_serialization():
    ...
    result = ToolResult(
        content=[create_text_content("test")],
        structuredContent=[StructuredContent(type="structured", data={"result": True})],
        isError=False,
    )
...
def test_empty_structured_content_data():
    ...
    content = StructuredContent(type="structured", data={})
...
def test_tool_result_validation_edge_cases():
    ...
    result3 = ToolResult(
        content=[], structuredContent=[StructuredContent(type="structured", data={})]
    )
    assert validate_tool_result(result3) is True
...
async def test_full_tool_workflow():
    ...
    async def math_handler(args):
        ...
        text_content = [
            create_text_content(f"Calculated {operation} of {numbers}: {result_value}")
        ]
