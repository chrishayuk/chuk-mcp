...
async def test_send_ping_success():
    ...
    async with anyio.create_task_group() as tg:
        ...
        # Client side ping
        result = await send_ping(read_stream=read_receive, write_stream=write_send)
    ...
    async def error_server():
        try:
            ...
            response = JSONRPCMessage(
                id=req.id, error={"code": 500, "message": "Internal server error"}
            )
            await read_send.send(response)
        except Exception as e:
...
async def test_send_ping_method_enum():
    ...
    async with anyio.create_task_group() as tg:
        ...
        # Perform ping
        await send_ping(read_stream=read_receive, write_stream=write_send)
