...
class DemoToolRegistry(ToolRegistry):
            ...
            Tool(
                ...
                inputSchema=ToolInputSchema(
                    properties={
                        ...
                        "operations": {
                            "type": "array",
                            "items": {"type": "string", "enum": ["stats", "outliers", "distribution"]},
    ...
    async def _analyze_text_handler(self, args: Dict[str, Any]) -> ToolResult:
        ...
        summary = (f"Analyzed text: {len(words)} words, {len(sentences)} sentences. "
    ...
    async def _get_weather_handler(self, args: Dict[str, Any]) -> ToolResult:
        ...
        weather_data = {
            ...
            "conditions": random.choice(["sunny", "partly_cloudy", "cloudy", "rainy", "snowy"]),
            "uv_index": random.randint(0, 11)
        }
        
        # Create weather summary
        summary = (f"Weather in {location}: {display_temp}{temp_unit}, "
...
async def demonstrate_tool_features():
    ...
    result = await registry.call_tool("analyze_text", {
        "text": sample_text,
    ...
    print(f"      • Sentiment: {structured_data['sentiment']['label']} ({structured_data['sentiment']['score']:.3f})")
    print(f"      • Keywords: {', '.join(structured_data['keywords'])}")
    
    # Demo 3: Weather data (mixed content)
    print("\n3️⃣  Weather Data Tool (Mixed Content)")
    print("-" * 42)
    result = await registry.call_tool("get_weather", {
    ...
    print(f"      • Temperature: {weather_data['temperature']['celsius']}°C / {weather_data['temperature']['fahrenheit']}°F")
    print(f"      • Conditions: {weather_data['conditions']}")
    print(f"      • Humidity: {weather_data['humidity']}%")
    print(f"      • Wind: {weather_data['wind']['speed']} km/h {weather_data['wind']['direction']}")
    ...
    result = await registry.call_tool("process_dataset", {
        "data": sample_data,
        "operations": ["stats", "outliers", "distribution"]
    })
    ...
    print(f"      • Memory: {sys_data['memory']['used_mb']}/{sys_data['memory']['total_mb']} MB used")
    print(f"      • CPU: {sys_data['cpu']['usage_percent']}% usage ({sys_data['cpu']['cores']} cores)")
    
    if "disk" in sys_data:
        print(f"      • Disk: {sys_data['disk']['used_gb']}/{sys_data['disk']['total_gb']} GB used")
    ...
    result = await registry.call_tool("analyze_text", {"text": "Test schema validation"})
        ...
        try:
            ...
            if command == "quit":
                ...
                if tool_name in registry.tools:
                    ...
                    print(f"Input Schema: {json.dumps(tool.inputSchema.model_dump(), indent=2)}")
                else:
