...
from chuk_mcp.mcp_client.messages.prompts.send_messages import send_prompts_list, send_prompts_get
    ...
    try:
        ...
        async with stdio_client(server_params) as (read_stream, write_stream):
            ...
            calc_response = await send_tools_call(
                read_stream, write_stream, "calculate", 
            ...
            todo_response = await send_tools_call(
                read_stream, write_stream, "add_todo", 
            ...
            status_response = await send_resources_read(read_stream, write_stream, "test://status")
            print(f"   📄 Status:\\n{status_response['contents'][0]['text']}")
            
            print("\\n   📖 Reading todos...")
            todos_response = await send_resources_read(read_stream, write_stream, "test://todos")
            ...
            math_prompt_response = await send_prompts_get(
                read_stream, write_stream, "explain_math", {"operation": "multiplication"}
            )
        ...
        async with stdio_client(server_params) as (read_stream, write_stream):
            ...
            async with anyio.create_task_group() as tg:
                ...
                async def counter_task(i):
                    ...
                    results.append(f"Counter {i+1}: {response['content'][0]['text'].split()[-1]}")
                
                async def random_task(i):
                    response = await send_tools_call(
                        read_stream, write_stream, "random_number", {"min": 1, "max": 10}
                    )
            ...
            end_time = time.time()
            print(f"   📊 Completed 6 concurrent operations in {end_time - start_time:.2f}s")
            ...
            async with stdio_client(server_params) as (read_stream, write_stream):
                ...
                async with anyio.create_task_group() as tg:
                    ...
                    async def counter_op():
                        ...
                        results.append("counter" in result["content"][0]["text"].lower())
...
async def run_performance_tests():
    ...
    try:
        ...
        print("\\n🎯 Performance Summary:")
        print(f"   • Connection setup: {'Fast' if avg_connection < 1.0 else 'Slow'} ({avg_connection:.3f}s)")
        print(f"   • Request throughput: {'Good' if concurrent_throughput > 50 else 'Needs improvement'} ({concurrent_throughput:.1f} req/s)")
