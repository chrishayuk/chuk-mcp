...
class SSECompatibilityTester:
        ...
        async with httpx.AsyncClient(headers=headers, timeout=10.0) as client:
            ...
            try:
                ...
                if sse_response.status_code == 200:
                    if "text/event-stream" in sse_response.headers.get(
                        "content-type", ""
                    ):
                        ...
                        results["errors"].append(
                            "SSE endpoint exists but doesn't return event-stream"
                        )
                else:
                    results["errors"].append(
                        f"SSE endpoint returned {sse_response.status_code}"
                    )
            ...
            try:
                ...
                if streamable_response.status_code in [200, 202]:
                    ...
                    results["errors"].append(
                        f"Streamable HTTP test returned {streamable_response.status_code}"
                    )
            ...
            async with sse_client(sse_params) as (read_stream, write_stream):
                ...
                try:
                    init_result = await send_initialize(
                        read_stream, write_stream, timeout=10.0
                    )
                    ...
                    logger.info(
                        f"✅ Initialization successful: {init_result.serverInfo.name}"
                    )

                    # Test ping to determine response pattern
                    ping_start = asyncio.get_event_loop().time()
                    ping_success = await send_ping(
                        read_stream, write_stream, timeout=5.0
                    )
                    ...
                    if ping_success:
                        ...
                        logger.info(
                            f"✅ Ping successful ({ping_duration:.3f}s) - {results['response_pattern']}"
                        )
                    ...
                    try:
                        ...
                        logger.info(
                            f"✅ Tools list: {results['tools_available']} tools available"
                        )
                    except Exception as e:
    ...
    async def run_migration_guidance(self) -> None:
        ...
        print(
            f"   Streamable HTTP: {'✅' if capabilities['streamable_http_supported'] else '❌'}"
        )
        ...
        if (
            capabilities["transport_recommendation"]
            == "both_supported_prefer_streamable"
        ):
            print("   ✅ Server supports both transports")
            print(
                "   💡 Recommend migrating to Streamable HTTP for future compatibility"
            )
            print(
                "   📌 SSE transport will continue to work for backwards compatibility"
            )
        ...
        if capabilities["sse_supported"]:
            ...
            if sse_results["connection_successful"]:
                ...
                if sse_results["initialization_successful"]:
                    ...
                    print(
                        f"   🏷️  Server: {sse_results['server_info']['name']} v{sse_results['server_info']['version']}"
                    )
                    ...
                    if sse_results["response_pattern"] == "immediate_http":
                        print(
                            "   💡 Server uses immediate HTTP responses (like your example)"
                        )
        ...
        print("\n📅 MIGRATION TIMELINE:")
        print("   • SSE transport deprecated: March 26, 2025 (MCP spec 2025-03-26)")
        print("   • SSE backwards compatibility: Maintained in chuk-mcp")
        print("   • Recommended action: Plan migration to Streamable HTTP")
        print("   • Support timeline: SSE will work but is not future-proof")
...
async def run_examples():
    ...
    try:
        ...
        async with try_sse_with_fallback(url="http://localhost:8000", timeout=10.0) as (
            read_stream,
            write_stream,
        ):
    ...
    for url in test_urls:
        ...
        try:
            ...
            capabilities = await asyncio.wait_for(
                tester.test_server_capabilities(), timeout=5.0
            )

            status = (
                "✅"
                if capabilities["sse_supported"]
                or capabilities["streamable_http_supported"]
                else "❌"
            )
            print(f"  {status} {capabilities['transport_recommendation']}")

        except asyncio.TimeoutError:
            print("  ⏱️  Timeout (server not available)")
        except Exception as e:
            print(f"  ❌ Error: {e}")
