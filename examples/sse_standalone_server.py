...
class ServerState:
    def __init__(self):
        ...
        self.server_info = {"name": "sse-example-server", "version": "1.0.0"}
        self.capabilities = {
...
async def handle_message(request: Request):
    ...
    if not session_id or session_id not in state.sessions:
        return JSONResponse(
            status_code=400, content={"error": "Invalid or missing session_id"}
        )
    ...
    try:
        ...
        logger.info(
            f"📨 Received message: {message.get('method')} (session: {session_id})"
        )

        response = await handle_mcp_message(message, session)
        if response:
            logger.info(
                f"📤 Sending response via SSE: {response.get('result', response.get('error'))}"
            )
    ...
    try:
        if method == "initialize":
            ...
            if tool_name == "sse_greet":
                ...
                greeting = (
                    f"🌊 {custom_message}, {name}! Greetings via SSE transport! 📡"
                )
                ...
                return {
                    ...
                    "result": {
                        "content": [
                            {
                                "type": "text",
                                "text": f"🔢 SSE Counter: {session['counter']} (+{increment})",
                            }
                        ]
                    },
                }
