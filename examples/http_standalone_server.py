...
class ServerState:
    def __init__(self):
        self.server_info = {"name": "http-example-server", "version": "1.0.0"}
        self.capabilities = {
...
async def handle_mcp_message(request: Request):
    ...
    try:
        ...
        logger.info(
            f"Received HTTP MCP message: {message.get('method')} (request #{state.request_count})"
        )
        ...
        if response:
            logger.info(
                f"Sending HTTP response: {response.get('result', response.get('error'))}"
            )
            return response
        else:
...
async def handle_mcp_message_logic(
        ...
        if method == "initialize":
            ...
            if tool_name == "http_greet":
                ...
                if action == "increment":
                    ...
                    state.counters[session_id] = 0
                    result_text = "🔢 HTTP Counter reset to 0"
                elif action == "get":
                    result_text = (
                        f"🔢 HTTP Counter current value: {state.counters[session_id]}"
                    )
                else:
                ...
                return {
                    ...
                    "result": {
                        "content": [
                            {
                                "type": "text",
                                "text": f"📝 HTTP Note #{note['id']} added with {priority} priority: {content}",
                            }
                        ]
                    },
                }
...
"""
                    ...
                    "prompts": [
                        {
                            ...
                            "arguments": [
                                {
                                    "name": "include_notes",
                                    "description": "Whether to include notes in summary",
                                    "required": False,
                                }
                            ],
                        },
                        {
                            ...
                            "arguments": [
                                {
                                    "name": "format",
                                    "description": "Documentation format (markdown/json)",
                                    "required": False,
                                }
                            ],
                        },
            ...
            if prompt_name == "http_session_summary":
                ...
                if include_notes and session_data["notes"]:
                    base_prompt += (
                        f" The session includes {len(session_data['notes'])} notes. "
                    )
                    base_prompt += "Notes:\n"
                ...
                if format_type == "json":
                    ...
                    prompt_text = "Please create comprehensive Markdown documentation for this HTTP MCP server API, covering all endpoints, request/response formats, and usage examples."
