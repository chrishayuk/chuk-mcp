        ...
        async with http_client(http_params) as (read_stream, write_stream):
            ...
            if any(t['name'] == 'http_greet' for t in tools):
                ...
                greet_result = await send_tools_call(
                    ...
                    "http_greet", {"name": "HTTP User", "style": "casual"}
                )
            ...
            if any(t['name'] == 'session_info' for t in tools):
                ...
                try:
                    ...
                    print(f"      Session ID: {session_data.get('session_id', 'Unknown')[:8]}...")
                    print(f"      Transport: {session_data.get('transport', 'Unknown')}")
                except:
                    print(f"      {content[:100]}...")
                ...
                for resource in resources[:2]:  # Test first 2
                    ...
                    if contents:
                        ...
                        if resource['mimeType'] == 'application/json':
                            print(f"        📄 JSON data available ({len(content_text)} chars)")
                        else:
...
async def fallback_example():
    ...
    try:
        ...
        async with http_client(http_params) as (read_stream, write_stream):
            init_result = await send_initialize(read_stream, write_stream)
            print(f"   ✅ Streamable HTTP connection successful!")
            print(f"   📋 Server: {init_result.serverInfo.name}")
    ...
    try:
        ...
        if transport_type in ["streamable_http", "both"]:
            ...
            async with http_client(params) as (read_stream, write_stream):
                init_result = await send_initialize(read_stream, write_stream)
                print(f"   ✅ Connected via Streamable HTTP!")
                print(f"   📋 Server: {init_result.serverInfo.name}")
        else:
...
async def parameter_creation_example():
    ...
    print(f"      Auth: {'✅ Bearer token set' if params3.bearer_token else '❌ No auth'}")
    ...
    try:
        async with http_client(params1) as (read_stream, write_stream):
            init_result = await send_initialize(read_stream, write_stream)
            print(f"      ✅ Connection successful with params1!")
            print(f"      📋 Server: {init_result.serverInfo.name}")
...
async def error_handling_example():
    ...
    try:
        malformed_params = StreamableHTTPParameters(
            url="not-a-valid-url",
            timeout=5.0
        )
        print(f"      ❌ This should have failed validation")
    ...
    try:
        with redirect_stderr(open(os.devnull, 'w')):
            ...
            async with http_client(timeout_params) as (read_stream, write_stream):
                await send_initialize(read_stream, write_stream)
        print(f"      ❌ This should have timed out")
