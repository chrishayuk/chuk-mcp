        ...
        class QuietHandler(logging.Handler):
            ...
            def emit(self, record):
                if record.levelno >= logging.ERROR:
                    ...
                    if "cancel scope" in msg.lower() or "json object must be str" in msg.lower():
                        self.tracker.errors.append({
                            'type': 'cancel_scope' if 'cancel scope' in msg.lower() else 'json_error',
                            'message': msg,
                            'logger': record.name
                        })
...
async def test_multiple_iterations(server_file: str, count: int = 10) -> tuple[int, int]:
...
async def test_concurrent_executions(server_file: str, count: int = 5) -> tuple[int, int]:
...
async def run_focused_tests():
    ...
    try:
        ...
        print(f"✅ {success_count}/{total} PASS" if success else f"❌ {success_count}/{total} FAIL")
