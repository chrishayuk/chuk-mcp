...
def main(
    src: str, include_deps: bool, show_breakdown: bool, include_tools: bool
) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_s:
        ...
        runtime_dists = [
            (n, sz)
            for n, sz in dists
            if include_tools or canonical(n) not in {canonical(t) for t in BUILD_TOOLS}
        ]
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Measure on-disk size of a Python package built from source."
    )
    p.add_argument("source", help="Path, git/https URL, or anything pip understands.")
    p.add_argument(
        "--no-deps",
        action="store_true",
        help="Skip installing dependencies inside the tmp venv.",
    )
    p.add_argument(
        "--breakdown",
        "-b",
        action="store_true",
        help="Show per-package size contribution (runtime only).",
    )
    p.add_argument(
        "--include-tools",
        action="store_true",
        help="Include build tools (pip/setuptools/wheel) in totals and table.",
    )
    args = p.parse_args()
    main(
        args.source,
