...
def test_without_pydantic():
    ...
    try:
        ...
        _model3 = TestModel(name="test3", extra_field="should_be_allowed")
        print("   • Extra fields: ✅ Allowed")
...
def test_message_compatibility():
    ...
    for mode, force_fallback in [("pydantic", False), ("fallback", True)]:
        ...
        try:
            ...
            for msg_data in test_messages:
                ...
                mode_results.append(
                    {"original": msg_data, "parsed": serialized, "json": json_str}
                )
    ...
    if results.get("pydantic") and results.get("fallback"):
        compatible = True
        for i, (pyd_result, fall_result) in enumerate(
            zip(results["pydantic"], results["fallback"])
        ):
    ...
    for mode, extra_args in [
        ...
        try:
            ...
            server_params = StdioParameters(
                command="python", args=[server_file] + extra_args
            )
...
def install_test():
    ...
    try:
        ...
        _params = StdioParameters(command="echo", args=["test"])
