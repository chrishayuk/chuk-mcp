...
class CancelScopeErrorHunter:
    ...
    def analyze_results(self):
        ...
        critical_errors = [
            e
            for e in self.cancel_scope_errors
            if e["type"] == "CRITICAL_CANCEL_SCOPE_ERROR"
        ]
        error_level = [
            e
            for e in self.cancel_scope_errors
            if e["type"] == "ERROR_LEVEL_CANCEL_SCOPE"
        ]
        debug_level = [
            e
            for e in self.cancel_scope_errors
            if e["type"] == "DEBUG_LEVEL_CANCEL_SCOPE"
        ]
...
async def run_cancel_scope_proof_tests():
    ...
    print("=" * 60)
    print(
        "Specifically hunting for: 'Attempted to exit cancel scope in a different task'"
    )
    print("=" * 60)
    ...
    try:
        ...
        for test_name, test_func in tests:
            ...
            # Report results
            if result["success"]:
                print("    ✅ PASS - No critical cancel scope errors")
            else:
                print(
                    f"    ❌ FAIL - {result['critical_errors']} critical, {result['error_level_messages']} error-level"
                )

            if result["total_cancel_scope_messages"] > 0:
                print(
                    f"    📊 Total cancel scope messages: {result['total_cancel_scope_messages']}"
                )
                print(
                    f"        Critical: {result['critical_errors']}, Error-level: {result['error_level_messages']}, Debug: {result['debug_level_messages']}"
                )
        ...
        if total_critical_errors == 0 and total_error_level == 0:
            ...
            print(
                "✅ The specific error 'Attempted to exit cancel scope in a different task' has been eliminated"
            )
            ...
            if total_debug > 0:
                print(
                    f"📝 {total_debug} DEBUG-level cancel scope messages detected (expected/harmless)"
                )
            ...
            for test_name, result in all_results:
                if result["critical_errors"] > 0 or result["error_level_messages"] > 0:
                    print(f"\n❌ {test_name} errors:")
                    for error in (
                        result["critical_error_details"] + result["error_level_details"]
                    ):
