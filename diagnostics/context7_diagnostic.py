...
class Context7Client:
    ...
    async def connect_and_test(self):
        ...
        try:
            async with http_client(http_params) as (read_stream, write_stream):
                ...
                print(f"   Server: {self.server_info.name} v{getattr(self.server_info, 'version', 'Unknown')}")
    ...
    async def test_tools(self, read_stream, write_stream):
        ...
        resolve_tool = next((t for t in self.tools if t['name'] == 'resolve-library-id'), None)
        if resolve_tool:
                    ...
                    if content:
                        ...
                        for line in lines:
                            if '/' in line and line.strip().startswith('/'):
                                ...
                                lib_id = line.split()[0] if ' ' in line else line.strip()
        ...
        docs_tool = next((t for t in self.tools if t['name'] == 'get-library-docs'), None)
        if docs_tool:
            ...
            for test_case in test_cases[:2]:  # Test first 2
                ...
                print(f"      Library: {test_case['params']['context7CompatibleLibraryID']}")
                ...
                try:
                    ...
                    if "not found" in error_msg.lower():
                        print(f"   ⚠️ Library not found - may need different ID")
                    else:
            ...
            try:
                ...
                content = self.extract_content(result)
                if "not found" in (content or "").lower() or "error" in (content or "").lower():
