...
def create_request(method: str, params: Optional[Dict[str, Any]] = None, 
                  ...
                  progress_token: Optional[ProgressToken] = None) -> JSONRPCRequest:
...
def create_notification(method: str, params: Optional[Dict[str, Any]] = None) -> JSONRPCNotification:
...
def create_error_response(id: RequestId, code: int, message: str, 
...
def parse_message(data: Union[Dict, List]) -> Union[JSONRPCRequest, JSONRPCNotification, JSONRPCResponse, JSONRPCError, List]:
    """
...
class JSONRPCMessageWrapper:
    ...
    def model_dump(self, **kwargs) -> Dict[str, Any]:
        ...
        if hasattr(self._message, 'model_dump'):
            ...
            # Batch message
            return [m.model_dump(**kwargs) for m in self._message]
        else:
...
class JSONRPCMessage(McpPydanticBase):
    ...
    def to_specific_type(self) -> Union[JSONRPCRequest, JSONRPCNotification, JSONRPCResponse, JSONRPCError]:
        """Convert to the appropriate specific message type."""
        if self.method is not None and self.id is not None:
            return JSONRPCRequest(jsonrpc=self.jsonrpc, id=self.id, method=self.method, params=self.params)
        elif self.method is not None and self.id is None:
            return JSONRPCNotification(jsonrpc=self.jsonrpc, method=self.method, params=self.params)
        elif self.id is not None and self.result is not None:
            return JSONRPCResponse(jsonrpc=self.jsonrpc, id=self.id, result=self.result)
        elif self.id is not None and self.error is not None:
            return JSONRPCError(jsonrpc=self.jsonrpc, id=self.id, error=self.error)
        else:
            raise ValueError("Invalid JSON-RPC message structure")
    
    @classmethod
    def from_specific_type(cls, msg: Union[JSONRPCRequest, JSONRPCNotification, JSONRPCResponse, JSONRPCError]) -> 'JSONRPCMessage':
        """Create from a specific message type."""
        if isinstance(msg, JSONRPCRequest):
            return cls(jsonrpc=msg.jsonrpc, id=msg.id, method=msg.method, params=msg.params)
        elif isinstance(msg, JSONRPCNotification):
            return cls(jsonrpc=msg.jsonrpc, method=msg.method, params=msg.params)
        elif isinstance(msg, JSONRPCResponse):
            return cls(jsonrpc=msg.jsonrpc, id=msg.id, result=msg.result)
        elif isinstance(msg, JSONRPCError):
            return cls(jsonrpc=msg.jsonrpc, id=msg.id, error=msg.error)
        else:
