# chuk_mcp/mcp_client/messages/sampling/send_messages.py
"""
...
from chuk_mcp.mcp_client.messages.content_types import (
    Content, TextContent, ImageContent, AudioContent,
    create_text_content, create_image_content, create_audio_content,
    Role
)
...
def create_sampling_message(
    ...
    if isinstance(content, str):
        content_obj = create_text_content(content)
    else:
        content_obj = content
    
    return SamplingMessage(role=role, content=content_obj)
...
async def sample_conversation(
    ...
    """
    messages = [
        create_sampling_message(role, content)
...
class SamplingHandler:
    ...
    async def handle_create_message_request(
        ...
        return {
            "role": result.role,
            "content": result.content.model_dump() if hasattr(result.content, 'model_dump') else result.content,
            "model": model_name,
